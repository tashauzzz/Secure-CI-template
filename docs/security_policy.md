...
This means that the full security pipeline must pass before a pull request can be merged into `main`.

Scheduled drift checks, such as `Runtime nightly`, are not configured as merge-required checks.
...
## 7. Known upstream-constrained dev-tooling advisory
